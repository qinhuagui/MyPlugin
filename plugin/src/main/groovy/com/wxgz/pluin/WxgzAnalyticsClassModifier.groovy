...
class WxgzAnalyticsClassModifier {
    ...
    private static byte[] modifyClass(byte[] srcClass) throws IOException {
        ...
        ClassVisitor classVisitor = new WxgzAnalyticsClassVisitor(classWriter)
...
//            println("------------过滤-------------- "+className+"   -----  "+className.startsWith(packageName)+  "   --- packageName = "+packageName)
            if (className.startsWith(packageName)) {
//                println("------------过滤--------------2222   mInterfaces = "+className.toString())
                return false
            }
        }
