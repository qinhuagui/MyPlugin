...
class WxgzAnalyticsClassVisitor extends ClassVisitor implements Opcodes {
    private final
    static String SDK_API_CLASS = "com/wxgz/sdk/asmapi/WxgzDataAutoTrackHelper"
    private String[] mInterfaces
    private ClassVisitor classVisitor

    private HashMap<String, WxgzAnalyticsMethodCell> mLambdaMethodCells = new HashMap<>()

    WxgzAnalyticsClassVisitor(final ClassVisitor classVisitor) {
    ...
    MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        ...
        methodVisitor = new WxgzAnalyticsDefaultMethodVisitor(methodVisitor, access, name, desc) {
            boolean isWxgzDataTrackViewOnClickAnnotation = false
            ...
            void visitInvokeDynamicInsn(String name1, String desc1, Handle bsm, Object... bsmArgs) {
                ...
                try {
                    ...
                    WxgzAnalyticsMethodCell wxgzAnalyticsMethodCell = WxgzAnalyticsHookConfig.LAMBDA_METHODS.get(Type.getReturnType(desc1).getDescriptor() + name1 + desc2)
                    if (wxgzAnalyticsMethodCell != null) {
                        Handle it = (Handle) bsmArgs[1]
                        mLambdaMethodCells.put(it.name + it.desc, wxgzAnalyticsMethodCell)
                    }
                } catch (Exception e) {
                    e.printStackTrace()
                }
            ...
            protected void onMethodEnter() {
                 ...
                 */
                WxgzAnalyticsMethodCell lambdaMethodCell = mLambdaMethodCells.get(nameDesc)
                if (lambdaMethodCell != null) {
                    ...
                    boolean isStaticMethod = WxgzAnalyticsUtils.isStatic(access)
                    if (!isStaticMethod) {
                ...
                }else if ( nameDesc == 'onPageFinished(Landroid/webkit/WebView;Ljava/lang/String;)V') {
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitVarInsn(ALOAD, 2)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SDK_API_CLASS, "trackWebViewPageFinished", "(Landroid/webkit/WebView;Ljava/lang/String;)V", false)
                }
                if (isWxgzDataTrackViewOnClickAnnotation) {
                ...
                if ((mInterfaces != null && mInterfaces.length > 0)) {
                    if ((mInterfaces.contains('android/view/View$OnClickListener') && nameDesc == 'onClick(Landroid/view/View;)V')) {
                        ...
                        methodVisitor.visitVarInsn(ALOAD, 1)
                        methodVisitor.visitVarInsn(ALOAD, 2)//ALOAD表示对象
                        methodVisitor.visitVarInsn(ILOAD, 3)//ILOAD是指参数为基本类型int、long、float，3表示插入方法体中的第三个参数
...
//                if (nameDesc.contains('WebViewClient')||nameDesc.contains('WebView')){
//                    println("------------android/webkit/WebViewClient--------------   mInterfaces = "+nameDesc.toString())
//                }
            }

            @Override
            AnnotationVisitor visitAnnotation(String s, boolean b) {
                if (s == 'Lcom/wxgz/sdk/asmapi/WxgzDataTrackViewOnClick;') {
                    isWxgzDataTrackViewOnClickAnnotation = true
                }
